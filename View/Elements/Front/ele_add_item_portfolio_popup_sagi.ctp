	...
	jQuery(function($){
        ...
        new AjaxUpload(btnUpload, {		
            ...
            onSubmit: function(file, ext){
                ...
                if (! (ext && /^(jpg|png|jpeg|gif|ppt|pptx|xls|xlsx|pdf|doc|docx|ppt|pptx)$/.test(ext))){
                    // check for valid file extension
                    status.text('Image and documents files are allowed.').addClass('errorTxt');
                    return false;
                }
            ...
            onComplete: function(file, response){
               ...
               // status.text('');
                ...
                if(myimageresponse[0]==="success"){
					if(myimageresponse[1]=='users/doc.png'){
					 $('#imageshow').html('<img src="<?php echo(SITE_URL); ?>/img/'+myimageresponse[1]+'" height="38" width="52"/><input type="hidden" name="data[UserPortfolio][image]" value="'+myimageresponse[1]+'"/><input type="hidden" name="data[UserPortfolio][user_id]" value="'+myimageresponse[3]+'"/><input type="hidden" name="data[UserPortfolio][id]" value="'+myimageresponse[2]+'"/>');
					}else{
							...
							<div class="PoupInFrm">
							  <ul class="PoupAddFrm">
								...
								<li>
								  ...
								  <span class="slct_rwndInPut">
								   ...
								   <?php //echo $this->Form->input('category_id', array('div'=>false, 'label'=>false, "class" => "slct_rwndInPutRi with_sml1","empty"=>'Select Category'));?>
									 <select name="data[UserPortfolio][category_id]" class="slct_rwndInPutRi with_sml1" id="UserPortfolioCategoryId">
										<option value="">-- Select Category --</option>
										<option value="67">Mentor</option>
										<option value="68">Advisor</option>
										<option value="61">Developer</option>
										<option value="62">Designer</option>
										<option value="63">Marketing</option>
										<option value="64">Sales</option>
										<option value="65">Product</option>
										<option value="66">Business Developer</option>
										<option value="88">Engineer</option>
										<option value="89">Writer</option>
										<option value="90">Music</option>
										<option value="91">Films</option>
										<option value="96">Photography</option>
										</select> 	
								  </div>
								  </span>
								</li>
								<li>
								  ...
								  <input type="button" name="" class="upload_btn" value="Upload" id="upload">
								  <span id="status"></span>
								...
								<li>
								  ...
								  <div class="addportfolio">
								 <?php echo $this->Form->input('description', array('div'=>false,'type'=>'textarea','label'=>false, "class" => "Description","maxlength"=>"60"));?> </div>	
								</li>
								<li>
								  <label>&nbsp;</label>
